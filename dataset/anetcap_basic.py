...
class AnetCapBasicDatasetBuilder():
    def __init__(self,cfg,data_path=None,anno_path=None,vid_path=None):
        ...
        # make paths
        if len(data_path) > 0:
            self.vid_path = join(data_path,"sub_activitynet_v1-3.c3d.hdf5")
            self.anno_path = {"train": join(data_path,"normal_set","captions","train_simplified_sentence.json"),
                                "test": [join(data_path,"normal_set","captions","val_1_simplified_sentence.json"),
                                        join(data_path,"normal_set","captions","val_2_simplified_sentence.json")]}
        if len(vid_path) > 0:
            self.vid_path = vid_path
        if len(anno_path) > 0:
            self.anno_path = anno_path
            self.anno_path['test'] = [self.anno_path['test'].format(i) for i in [1,2]]
        # read annotations
